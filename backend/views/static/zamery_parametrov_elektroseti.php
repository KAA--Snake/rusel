...
<div class="inner_wrap mw1180">
    ...
    <h1 class="header_h1">Форма для редактирования страницы "Замеры параметров электросети"</h1>

    <form action="/admin/static/add/" method="post" id="static_add-form" class="static_add-form" enctype="multipart/form-data">

        <input type="hidden" name="Artikle[type]" value="<?=$model->type;?>" />
        <input type="hidden" name="Artikle[id]" value="<?=$model->id;?>" />

        <input type="text" name="Artikle[name]" value="Замеры параметров электросети" />
